    ...
    <div className={`${isSidebarOpen ? 'w-64' : 'w-20'} transition-all duration-300 h-screen sticky top-0 bg-slate-50 dark:bg-black border-r border-slate-200 dark:border-white/10 flex flex-col shrink-0 overflow-y-auto overflow-x-hidden hidden md:flex`}>
      ...
      <div className="mt-auto p-4 border-t border-slate-200 dark:border-white/10 space-y-3">
        ...
        <button
          ...
          className={`w-full flex items-center ${isSidebarOpen ? 'justify-between px-3' : 'justify-center px-0'} py-2 rounded-lg text-sm font-medium border ${
            zenMode 
              ? "bg-amber-500/10 border-amber-500/20 text-amber-400" 
              : "border-slate-300 dark:border-white/10 text-slate-600 dark:text-slate-400 hover:bg-[#0a0a0a]"
          }`}
          title={!isSidebarOpen ? "Zen Mode" : undefined}
        >
          {isSidebarOpen ? (
            <>
              Zen Mode
              <span className="text-xs opacity-70">{zenMode ? 'ON' : 'OFF'}</span>
            </>
          ) : (
