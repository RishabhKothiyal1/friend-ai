...
const CHARACTERS: Character[] = [
  ...
  },
  {
    ...
    longDescription: "Hope welcomes you into a serene Paitkar scroll-painting room of Jharkhand. Surrounded by rich terracotta browns, yellow-ochre washes, and historic figures drawn with traditional, elongated folk-art eyes, Hope acts as a custodian of your personal narrative. She helps you unroll your worries like a beautiful, slowly-developing scroll of resilience.",
  ...
  },
  {
    ...
    longDescription: "Meet Altaf, our somatic media & security specialist styled in the exquisite Rogan Art of Kutch, Gujarat. Just as Rogan art relies on pulling cast-oil gel into perfect paint thread symmetry across a fold, Altaf guides you through aligning your physical posture, monitoring breathing sounds, and analyzing camera/voice data in a zero-trust safe space. He combines Gujarat's golden style with deep somatic reflection.",
  ...
  },
  {
    ...
    longDescription: "Dressed in a formal bandhgala, Veer sits within a glorious Odisha Pata Chitra library. Featuring heavily decorated borders, sharp black-ink outlines, and traditional tempera figures representing legal justice, Veer delivers deep legal precision. He empowers patients with transparent forensic tools, protection directories, and statutory guidance.",
  ...
  },
  {
    ...
    longDescription: "Manjishtha is our resident sharp-witted feline, perched inside a cozy attic decorated with Maharashtrian Warli tribal art. On rustic mud-washed deep brown walls, simple stick figures, triangular hunters, and musical loops dance in endless circles (the Tarpa dance) around her typewriting sill. Manjishtha delivers deep, lighthearted, yet cynical cat wisdom.",
...
export default function App() {
  ...
  const [gmailFilter, setGmailFilter] = useState<'all' | 'inbox' | 'sent' | 'starred'>('all');
  const [gmailSelectedEmail, setGmailSelectedEmail] = useState<GmailMessage | null>(null);
  const [gmailComposeTo, setGmailComposeTo] = useState<string>("");
  const [gmailComposeSubject, setGmailComposeSubject] = useState<string>("");
  const [gmailComposeBody, setGmailComposeBody] = useState<string>("");
  const [gmailStatusMessage, setGmailStatusMessage] = useState<{ type: 'success' | 'error', text: string } | null>(null);
  ...
  // Fetch emails when token or filters change
  const handleLoadEmails = async () => {
    if (!gmailToken) return;
    setGmailLoading(true);
    setGmailStatusMessage(null);
    try {
      const msgs = await fetchGmailEmails(gmailToken, gmailFilter, gmailSearch);
      setGmailEmails(msgs);
    } catch (err: any) {
      console.error(err);
      setGmailStatusMessage({ 
        type: 'error', 
        text: "Failed to fetch from Gmail API. Try signing out and signing in again to ensure authorizations are synced." 
      });
    } finally {
  ...
  const handleGmailSignIn = async () => {
    ...
    try {
      ...
      setGmailStatusMessage({ type: 'error', text: `Authentication failed: ${err.message || err}` });
    }
  };
  ...
  const handleSendGmail = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!gmailToken) return;
    if (!gmailComposeTo || !gmailComposeSubject || !gmailComposeBody) {
      setGmailStatusMessage({ type: 'error', text: "All composer fields are required." });
      return;
    }
    // MUTATION SAFEGUARD: Require an explicit user confirmation dialog
    const isConfirmed = window.confirm(`Are you sure you want to send this email to ${gmailComposeTo}?`);
    if (!isConfirmed) return;

    setIsSendingGmail(true);
    setGmailStatusMessage(null);
    try {
      await sendGmailEmail(gmailToken, gmailComposeTo, gmailComposeSubject, gmailComposeBody);
      setGmailStatusMessage({ type: 'success', text: `Email successfully sent to ${gmailComposeTo}!` });
      // Reset composer
      setGmailComposeTo("");
      setGmailComposeSubject("");
      setGmailComposeBody("");
      setShowGmailComposer(false);
      // Reload sent folder or current list
      handleLoadEmails();
    } catch (err: any) {
      console.error(err);
      setGmailStatusMessage({ type: 'error', text: `Failed to send email: ${err.message || err}` });
    } finally {
  ...
  const handleCreateGmailDraft = async () => {
    if (!gmailToken) return;
    if (!gmailComposeTo || !gmailComposeSubject || !gmailComposeBody) {
      setGmailStatusMessage({ type: 'error', text: "Please fill out details to draft." });
      return;
    }
    // MUTATION SAFEGUARD: Require an explicit user confirmation dialog
    const isConfirmed = window.confirm(`Draft a message in your Gmail account?`);
    if (!isConfirmed) return;

    setIsSendingGmail(true);
    setGmailStatusMessage(null);
    try {
      await createGmailDraft(gmailToken, gmailComposeTo, gmailComposeSubject, gmailComposeBody);
      setGmailStatusMessage({ type: 'success', text: "Draft successfully created in your Gmail!" });
      setShowGmailComposer(false);
      setGmailComposeTo("");
      setGmailComposeSubject("");
      setGmailComposeBody("");
      ...
      console.error(err);
      setGmailStatusMessage({ type: 'error', text: `Failed to save draft: ${err.message || err}` });
    } finally {
      setIsSendingGmail(false);
    }
  };

  const handleTrashGmailMessage = async (msgId: string) => {
    if (!gmailToken) return;
    // MUTATION SAFEGUARD: Require an explicit user confirmation dialog
    const isConfirmed = window.confirm("Are you sure you want to move this message to trash?");
    if (!isConfirmed) return;

    setGmailStatusMessage(null);
    try {
      await trashGmailEmail(gmailToken, msgId);
      setGmailStatusMessage({ type: 'success', text: "Message moved to Gmail trash." });
      if (gmailSelectedEmail?.id === msgId) {
        setGmailSelectedEmail(null);
      }
      handleLoadEmails();
    } catch (err: any) {
      console.error(err);
      setGmailStatusMessage({ type: 'error', text: `Failed to trash email: ${err.message || err}` });
    }
  };

  const shareComplianceDraft = () => {
    // Fill composer with helpful deescalation list or active counselor directory items to send to self/colleague
    setGmailComposeTo(gmailUser?.email || "");
    setGmailComposeSubject("Shared Clinical Guideline: Project Friend AI Ethics Checklist");
    setGmailComposeBody(
      `Hello,\n\nPlease find the following mental health ethical compliance details shared with you from Project Friend AI:\n\n` +
      `- Absolute Privacy-First Local sandboxing (verified)\n` +
      `- Comprehensive clinical boundary de-escalation\n` +
      `- Active counselor lists include Vandrevala Helpline & US Lifeline (988)\n\n` +
  ...
  // Auto-complete reading goal when entering blogs center tab
  useEffect(() => {
    if (activeCenterTab === 'blogs') {
      completeDailyGoal("goal_blog");
    }
  }, [activeCenterTab]);
  
  // Blog & Founder Portal State
  const [blogsList, setBlogsList] = useState<{
    ...
    {
      id: "preseeded-anxiety",
      title: "Sovereign Pathways: Reframing Anxiety and the Somatic Inhale",
      author: "Manjishtha Pahilajani, Founder",
      date: new Date(Date.now() - 2 * 24 * 60 * 60 * 1000).toLocaleDateString("en-US", { year: 'numeric', month: 'long', day: 'numeric' }),
      tags: ["Anxiety", "Somatic", "Grounding"],
      content: `# Sovereign Pathways: Reframing Anxiety and the Somatic Inhale
*By Manjishtha Pahilajani, Founder of Project Friend AI*  
*Published Academic Column — Weekly Wisdom Series*
...
**Take Your Next Breath with Sanctuary**: If you are seeking a quiet, fully encrypted, browser-sandboxed de-escalation workspace where you can safely switch companion companion personas without judgment, I invite you to join us inside **Project Friend AI**. We have crafted this space as a secure harbor for your mind. You are never alone.`
    },
    {
      id: "preseeded-burnout",
      title: "Dismantling Burnout: Structural Boundaries and Sympathetic Balance",
      author: "Manjishtha Pahilajani, Founder",
      date: new Date(Date.now() - 5 * 24 * 60 * 60 * 1000).toLocaleDateString("en-US", { year: 'numeric', month: 'long', day: 'numeric' }),
      tags: ["Burnout", "Work Stress", "Boundaries"],
      content: `# Dismantling Burnout: Structural Boundaries and Sympathetic Balance
*By Manjishtha Pahilajani, Founder of Project Friend AI*  
*Published Academic Column — Weekly Wisdom Series*
...
**Cultivate Your Personal Boundary**: If you need a deliberate, pressure-free sanctuary to step away from the corporate demands, I invite you to open **Project Friend AI**. Select one of our somatic guides, set a master volume sleep timer, and let yourself simply exist in a space that asks nothing from you.`
  ...
  const [blogTopicInput, setBlogTopicInput] = useState<string>("Somatic Regulation and Emotional Pacing");
  const [isGeneratingBlog, setIsGeneratingBlog] = useState<boolean>(false);
  const [customBlogTitle, setCustomBlogTitle] = useState<string>("");
  const [customBlogContent, setCustomBlogContent] = useState<string>("");
  const [customBlogTags, setCustomBlogTags] = useState<string>("Self-Care, Grounding");
  const [isFounderPanelOpen, setIsFounderPanelOpen] = useState<boolean>(false);
  const [founderPIN, setFounderPIN] = useState<string>("");
  const [isFounderAuthorized, setIsFounderAuthorized] = useState<boolean>(false);
  const [selectedAuthorPerspective, setSelectedAuthorPerspective] = useState<string>("Manjishtha Pahilajani, Founder");
  const [blogStatusMessage, setBlogStatusMessage] = useState<{ type: 'success' | 'error' | 'info'; text: string } | null>(null);
  ...
  const [dailyGoals, setDailyGoals] = useState<Array<{ id: string; label: string; actionText: string; isCompleted: boolean; icon: string; points: number }>>(() => {
    try {
      ...
      return saved ? JSON.parse(saved) : [
        ...
        { id: "goal_blog", label: "Read Curated Self-Care Column", actionText: "Visit any Wisdom Blog to learn tips", isCompleted: false, icon: "📖", points: 20 },
...
`;
  ...
  const handleCharacterChange = (charId: string) => {
    ...
    const welcomeText = `I have initialized specialized support. I am now speaking as **${target.name}**.`;
        ...
        <section className={`xl:col-span-12 flex flex-col rounded-2xl overflow-hidden min-h-[600px] ${
          ...
          {activeCenterTab === 'community' && (
            <div className={`flex-1 p-5 overflow-y-auto space-y-6 font-sans rounded-b-2xl h-[700px] xl:h-[750px] flex flex-col transition-all duration-300 ${themeClass("bg-white text-slate-800", "bg-black/60 text-slate-100", "bg-[#fdf9f0] text-[#3e2723]")}`}>
              ...
              <div className={`border p-5 rounded-2xl relative overflow-hidden flex flex-col gap-2.5 transition-all duration-300 ${themeClass("bg-[#fdf5f8] border-[#f4e2ea]", "bg-slate-950/40 border-white/10/40", "bg-[#ebdcb9]/30 border-[#ebdcb9]")}`}>
                ...
                <div className="flex flex-col gap-1 z-10 text-left">
                  ...
                  <h3 className={`text-base md:text-lg font-bold flex items-center gap-2 font-display ${themeClass("text-slate-900", "text-pink-200", "text-[#3e2723]")}`}>
                    Mindful Community Garden
                  </h3>
                  <p className={`text-xs leading-relaxed max-w-2xl ${themeClass("text-slate-600", "text-slate-350", "text-[#5c4033]/80")}`}>
                    Whenever you add a mood and show progress, you plant a digital botanical seed here. Seeds grow gracefully into lovely blooms to materialize your neurological resilience and healing journey.
                  </p>
                </div>
              </div>
              ...
              <div className={`p-5 rounded-2xl border transition-all duration-300 text-left relative overflow-hidden ${themeClass(
                  ...
                  <div className="flex items-center gap-2.5">
                    ...
                    <div>
                      <h4 className="text-xs font-bold uppercase tracking-wider text-slate-800 dark:text-slate-100 font-display">
                        Daily Self-Care Goals Checklist
                      </h4>
                      <p className="text-[10.5px] text-slate-550 dark:text-slate-400 mt-0.5 leading-normal">
                        Nourish your plants. Completed activities automatically spray growth catalyst to all your active garden seeds!
                      </p>
                    </div>
                  </div>
                ...
                {(() => {
                  ...
                  return (
                    <div className="py-3.5 flex flex-col gap-1.5">
                      ...
                      <div className="w-full bg-slate-150 dark:bg-slate-850 rounded-full h-2 overflow-hidden shadow-inner">
                        <motion.div 
                          className="h-full rounded-full bg-gradient-to-r from-teal-500 via-indigo-500 to-pink-500"
                          initial={{ width: "0%" }}
                ...
                {/* Grid checklist list */}
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-5 gap-3.5 mt-1.5">
                  {dailyGoals.map((goal) => {
                    return (
                      <div
                        key={goal.id}
                      ...
                      >
                        <div className="flex items-center gap-2">
                          <span className="text-lg shrink-0 select-none">{goal.icon}</span>
                          <span className={`text-[10.5px] font-bold leading-tight ${goal.isCompleted ? "line-through text-slate-400 dark:text-slate-500 font-normal" : "text-slate-750 dark:text-slate-200"}`}>
                            {goal.label}
                          </span>
                        </div>

                        <div className="mt-auto space-y-2 pt-2 border-t border-slate-100/40">
                          <span className="text-[9px] text-slate-450 dark:text-slate-550 leading-normal block">
                            {goal.actionText}
                          </span>
                          ...
                          <button
                            ...
                            onClick={() => {
                              ...
                              if (goal.id === "goal_breathe") {
                                toggleBreathing();
                              } else if (goal.id === "goal_mood") {
                                const el = document.getElementById("pfi-mood-tracker-anchor");
                                if (el) el.scrollIntoView({ behavior: "smooth" });
                                setGoalToast({
                                  show: true,
                                  title: "Complete Activity! 📊",
                                  text: "Drop a state update in the right-column Mood Tracker to log this goal automatically!"
                                });
                                setTimeout(() => setGoalToast(p => ({ ...p, show: false })), 4500);
                              } else if (goal.id === "goal_reflection") {
                                setActiveCenterTab("publishing");
                              } else if (goal.id === "goal_blog") {
                                setActiveCenterTab("blogs");
                              } else if (goal.id === "goal_zen") {
                                const btn = document.getElementById("zen-mode-toggle-btn");
                                if (btn) btn.click();
                              } else {
                                completeDailyGoal(goal.id);
                              }
                            }}
                            className={`w-full py-1 rounded-lg text-[9px] font-mono uppercase tracking-wide font-bold transition-all text-center flex items-center justify-center gap-1 cursor-pointer ${
                              goal.isCompleted
                          ...
                          >
                            <span>{goal.isCompleted ? "✓ Completed" : "⚡ Go Complete"}</span>
                          </button>
                        </div>
                      </div>
                    );
              ...
              <div className="flex-1 flex flex-col gap-5 text-left">
                <div className="flex items-center justify-between border-b pb-1.5 border-slate-150">
                  <div>
                    <h4 className="text-xs font-bold uppercase tracking-wider">Your Living Botanical Shelf</h4>
                    <p className="text-[9.5px] text-slate-450 mt-0.5">Vibrant flora visualizing your inner emotional milestones</p>
                  </div>
                  <div className="text-xs font-mono font-bold bg-pink-100/50 text-pink-700 px-3 py-1 rounded-full flex items-center gap-1">
                    🌳 Total Plants: {gardenFlowers.length}
                  </div>
                </div>
                ...
                {gardenFlowers.length === 0 ? (
                  <div className="py-20 text-center flex flex-col items-center justify-center gap-3">
                    ...
                    <p className="text-xs italic text-slate-400">Your garden is empty. Log a positive mood improvement to plant your first seed!</p>
                  </div>
                ) : (
                  <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-5">
                    {gardenFlowers.map((flower) => {
                      // Styling configuration based on flower types
                      const styleMap: Record<string, { emoji: string, color: string, badge: string, label: string }> = {
                        Lotus: { emoji: "🌸", color: "from-pink-400 to-rose-500", badge: "bg-pink-100 text-pink-700", label: "Pink Lotus" },
                        SunFlower: { emoji: "🌻", color: "from-yellow-400 to-amber-500", badge: "bg-amber-100 text-amber-800", label: "SunFlower" },
                        Lavender: { emoji: "🪻", color: "from-purple-400 to-indigo-500", badge: "bg-purple-100 text-purple-700", label: "Lavender Spike" },
                        Rose: { emoji: "🌹", color: "from-rose-500 to-red-650", badge: "bg-rose-100 text-rose-700", label: "Gilded Rose" },
                      };
                      const config = styleMap[flower.type] || styleMap.Lotus;
                      ...
                      return (
                        ...
                        >
                          <div className="absolute top-2 right-2 flex flex-col gap-1 items-end">
                            <span className={`text-[8.5px] font-mono px-1.5 py-0.2 rounded font-bold ${config.badge}`}>
                              {config.label}
                            </span>
                          </div>

                          {/* Animated flower container block */}
                          <div className="flex-1 flex items-center justify-center py-6 relative">
                            {/* Decorative aura rings */}
                            <div className={`absolute w-12 h-12 rounded-full opacity-10 animate-ping bg-gradient-to-tr ${config.color}`}></div>

                            {/* Growing flower avatar */}
                            <motion.div
                              initial={{ scale: 0.1, rotate: -20 }}
                              animate={{ scale: flower.scale, rotate: 0 }}
                              transition={{ duration: 1.8, ease: "easeOut" }}
                              className="text-5xl select-none relative z-10 transition-transform filter drop-shadow-md"
                            >
                              {config.emoji}
                            </motion.div>
                          </div>

                          {/* Flower details block */}
                          <div className="space-y-1 mt-auto">
                            <h5 className="font-extrabold text-xs truncate leading-tight">{flower.name}</h5>
                            <div className="flex items-center justify-between text-[9px] font-mono text-slate-400 mt-2">
                              <span>Planted:</span>
                              <span className="font-bold">{flower.timestamp}</span>
                            </div>
                            ...
                            <div className="flex flex-col gap-1 pt-1">
                              <div className="flex items-center justify-between text-[8px] font-mono text-slate-400">
                                <span>Growth:</span>
                                <span className={flower.scale < 1.0 ? "text-emerald-500 animate-pulse font-bold animate-pulse" : "text-slate-400"}>
            ...
            <div className={`mt-8 p-6 rounded-2xl border text-left transition-all duration-300 relative overflow-hidden ${themeClass(
              ...
              <div className="absolute top-0 right-0 p-3 pointer-events-none select-none text-2xl opacity-15">
                🌸🏡✨
              </div>

              <div className="max-w-xl space-y-2">
                <h4 className="text-xs font-black uppercase tracking-widest text-indigo-750 font-mono">
                  🌸 The Sanctuary Gathering
                </h4>
                <h3 className="text-sm font-bold leading-normal">
                  Our Whole Community Gathers to Nourish Your Living Garden
                </h3>
                <p className="text-[11px] text-slate-550 leading-relaxed">
                  Every unique guide and companion is gathered here in the center, working in warm harmony to care for, comfort, and tend to your blooming milestone flowers. Their combined strengths, music, and quiet wisdom watch over your progress.
                </p>
              </div>
                ...
                {[
                  {
                    ...
                    tending: "Gently holds space beneath the buds, breathing slow rhythm into the room to keep the air safe and welcoming.",
                    seedling: "Pink Lotus"
                  },
                  {
                    name: "Ganesh",
                    ...
                    tending: "Carefully waters the soil, gently clearing away dry weeds of 'all-or-nothing' cognitive distortions so roots can breathe.",
                    seedling: "SunFlower"
                  },
                  {
                    name: "Raag",
                    ...
                    tending: "Plays extremely safe, low-tempo acoustic ragas to the lavender stalks, relieving acoustic stress and heavy pacing waves.",
                  ...
                  },
                  {
                    ...
                    tending: "Lights rustic terracotta oil lamps (diya) nearby, sheltering the young leaves from cold wind with gentle heritage warmth.",
                  ...
                  },
                  {
                    ...
                    tending: "Sits entirely quietly by the window, letting her silence validate the seedlings' pace, reminding us that growing takes time.",
                  ...
                  },
                  {
                    ...
                    tending: "Verifies secure soil health, audits nutrient encryption parameters, and safeguards the sanctuary's strict user privacy.",
                  ...
                  },
                  {
                    ...
                    tending: "Carefully structures a durable protective glass frame around the pots to keep legal worries or system threats far away.",
                    seedling: "Guard Fern"
                  },
                  {
                    name: "Manjishtha",
                    ...
                    tending: "Curls up on the warm brick floor right beside your pots. *gentle purr* She makes sure no mice or cynical doubts bother the sprouts.",
                    seedling: "Catnip Bloom"
                  }
                ].map((member, idx) => (
                  <div 
                    key={idx} 
                  ...
                  >
                    <div className="space-y-2">
                      <div className="flex items-center gap-2">
                        <span className={`w-7 h-7 rounded-full flex items-center justify-center font-bold text-center text-xs border ${member.bgCol}`}>
                          {member.avatar}
                        </span>
                        <div>
                          <h4 className="text-[11.5px] font-extrabold leading-none">{member.name}</h4>
                          <span className="text-[8.5px] text-slate-400 font-mono block mt-0.5">{member.role}</span>
                        </div>
                      </div>
            ...
            <div className={`flex-1 p-5 overflow-y-auto space-y-6 font-sans rounded-b-2xl h-[700px] xl:h-[750px] transition-all duration-300 ${themeClass("bg-white text-slate-800", "bg-black/60 text-slate-100", "bg-[#fdf9f0] text-[#3e2723]")}`}>
              ...
              <div className={`border p-5 rounded-2xl relative overflow-hidden flex flex-col gap-3 transition-all duration-300 ${themeClass("bg-rose-50/40 border-rose-100", "bg-rose-950/10 border-rose-900/20", "bg-[#f5ebd6] border-[#e0cbab]")}`}>
                ...
                <div className="flex flex-col gap-1.5 z-10 text-left">
                  <span className="text-[9.5px] uppercase font-mono tracking-widest px-2.5 py-0.5 rounded-md font-bold bg-rose-600 text-white self-start">
                    Google Workspace Integration
                  </span>
                  <h3 className={`text-base md:text-lg font-bold flex items-center gap-2 font-display ${themeClass("text-slate-900", "text-rose-200", "text-[#3e2723]")}`}>
                    Secure Client-Side Gmail Portal
                  </h3>
                  <p className={`text-xs leading-relaxed max-w-2xl ${themeClass("text-slate-650", "text-slate-300", "text-[#5c4033]")}`}>
                    Directly access, draft, or dispatch reassuring clinical reminders, de-escalation logs, and emergency contact details.
                    To preserve complete privacy, authentication and tokens are fully sandboxed on-device and in-memory.
                  </p>
                </div>
              </div>
              ...
              {gmailStatusMessage && (
                <div className={`p-4 rounded-xl text-xs flex items-center gap-2 border animate-fade-in ${
                  gmailStatusMessage.type === 'success'
                    ...
                    : "bg-rose-50 text-rose-800 border-rose-200 dark:bg-rose-950/20 dark:text-rose-305 dark:border-rose-900/30"
                }`}>
                  <span className="text-sm font-bold">{gmailStatusMessage.type === 'success' ? "✓" : "⚠"}</span>
                  <p>{gmailStatusMessage.text}</p>
                </div>
              )}
              ...
              {needsGmailAuth ? (
                ...
                <div className={`border border-dashed p-10 rounded-2xl text-center flex flex-col items-center gap-5 justify-center ${themeClass("bg-slate-50/50 border-slate-200", "bg-slate-950/20 border-white/10", "bg-[#faf5eb] border-[#ebdcb9]")}`}>
                  ...
                  <div className="space-y-1.5 max-w-md">
                    <h4 className="text-sm font-bold text-slate-800 dark:text-slate-205">Connect Your Gmail Account</h4>
                    <p className="text-xs text-slate-500 leading-relaxed">
                      Authenticate with your Google account credentials safely to inspect journals, send emergency checklists to trusted caregivers, or keep records secure.
                    </p>
                  </div>
                  <button
                    onClick={handleGmailSignIn}
                    className="px-5 py-2.5 bg-rose-600 hover:bg-rose-700 text-white text-xs font-bold rounded-xl cursor-pointer shadow-sm transition-all flex items-center gap-2"
                  >
                    <svg className="w-4 h-4 fill-current" viewBox="0 0 24 24">
                      <path d="M12.24 10.285V13.4h6.887c-.275 1.565-1.88 4.604-6.887 4.604-4.33 0-7.859-3.578-7.859-8s3.529-8 7.859-8c2.46 0 4.105 1.025 5.047 1.926l2.427-2.334C17.955 2.192 15.34 1 12.24 1 6.033 1 1 6.033 1 12.24s5.033 11.24 11.24 11.24c6.478 0 10.793-4.537 10.793-10.986 0-.742-.08-1.3-.175-1.859H12.24z"/>
                    </svg>
                ...
                <div className="space-y-4">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="text-xs font-bold text-slate-800 dark:text-slate-100">
                          {gmailUser?.displayName || "Google Workspace User"}
                        </p>
                  ...
                  <div className="flex flex-wrap items-center gap-2 justify-between">
                    <div className="flex items-center gap-1">
                      {(['all', 'inbox', 'sent', 'starred'] as const).map((filterOpt) => (
                        <button
                          key={filterOpt}
                          onClick={() => {
                            setGmailFilter(filterOpt);
                            setGmailSelectedEmail(null);
                          }}
                    ...
                    <div className="flex items-center gap-2">
                      <button
                        ...
                        className="px-3.5 py-1.5 bg-rose-600 hover:bg-rose-700 text-white text-[11px] font-bold rounded-lg cursor-pointer transition-all flex items-center gap-1"
                      >
                  ...
                  <div className="relative">
                    ...
                    <button
                      ...
                      className="absolute right-1.5 top-1.5 bottom-1.5 px-3 bg-rose-600 font-bold hover:bg-rose-700 text-white text-[10px] rounded-lg transition-all cursor-pointer"
                    >
                      Search
                    </button>
                  </div>
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-12 gap-4">
                    ...
                    <div className="lg:col-span-5 space-y-2 max-h-[480px] overflow-y-auto pr-1">
                      {gmailLoading ? (
                        <div className="p-8 text-center text-xs text-slate-500 font-mono animate-pulse">
                          <span>Loading messages from Gmail API securely...</span>
                        </div>
                      ) : gmailEmails.length === 0 ? (
                        <div className="p-8 text-center border border-dashed rounded-xl text-xs text-slate-500">
                          No emails matching the selected folder or search query.
                        </div>
                      ) : (
                        gmailEmails.map((email) => {
                          const isSelected = gmailSelectedEmail?.id === email.id;
                          return (
                            <div
                              key={email.id}
                              onClick={() => {
                                setGmailSelectedEmail(email);
                                setShowGmailComposer(false);
                              }}
                            ...
                            >
                              <div className="flex items-start justify-between gap-2 mb-1.5">
                                <p className="font-sans font-semibold text-[11px] text-slate-800 dark:text-slate-200 truncate max-w-[130px]">
                              ...
                              <h5 className="font-bold text-[11.5px] text-slate-900 dark:text-slate-100 truncate mb-1">
                                {email.subject}
                              </h5>
                              ...
                              <div className="flex justify-between items-center mt-2.5 pt-2 border-t border-slate-100 dark:border-white/10/60 font-mono text-[8px] text-slate-400">
                                ...
                                <button
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    handleTrashGmailMessage(email.id);
                                  }}
                                  className="text-rose-500 hover:text-rose-700 dark:text-rose-300 hover:underline p-1 cursor-pointer transition-all"
                    ...
                    {/* Right Column: Detail Content OR Composer */}
                      ...
                      {showGmailComposer ? (
                        ...
                        <form onSubmit={handleSendGmail} className={`p-4 border rounded-xl space-y-3 text-left ${themeClass("bg-white border-slate-200", "bg-slate-950/20 border-white/10", "bg-[#faf5eb] border-[#ebdcb9]")}`}>
                          <div className="flex items-center justify-between border-b pb-2">
                            ...
                            <button
                              type="button"
                              onClick={() => setShowGmailComposer(false)}
                              className="text-slate-400 hover:text-slate-650 dark:text-slate-350 cursor-pointer"
                            >
                              ✕ Close
                            </button>
                          </div>
                          ...
                          <div className="space-y-1 text-xs">
                            <label className="block text-[10px] font-bold text-slate-500 uppercase font-mono">Recipient (To:)</label>
                            <input
                              type="email"
                              required
                          ...
                          <div className="space-y-1 text-xs">
                            <label className="block text-[10px] font-bold text-slate-500 uppercase font-mono">Subject</label>
                            <input
                              type="text"
                              required
                              className="w-full p-2 border border-slate-205 dark:border-white/10 bg-white dark:bg-black rounded-lg text-xs outline-none focus:border-rose-450"
                          ...
                          <div className="space-y-1 text-xs">
                            <label className="block text-[10px] font-bold text-slate-500 uppercase font-mono">Message body description</label>
                            <textarea
                              required
                              rows={8}
                          ...
                          <div className="flex items-center gap-2 pt-2 justify-end">
                            ...
                            <button
                              type="submit"
                              disabled={isSendingGmail}
                              className="px-4 py-2 bg-rose-600 hover:bg-rose-700 text-white text-xs font-bold rounded-lg cursor-pointer shadow-sm transition-all"
                            >
                              {isSendingGmail ? "Sending..." : "📤 Send Message"}
                            </button>
                          </div>
                        </form>
                      ) : gmailSelectedEmail ? (
                        /* Detailed Email View */
                        <div className={`p-4 border rounded-xl space-y-4 text-left ${themeClass("bg-white border-[#fce7e7]", "bg-slate-950/20 border-rose-950/20", "bg-[#f5ebd6] border-[#e0cbab]")}`}>
                          <div className="flex items-start justify-between border-b pb-3 gap-4">
                            <div>
                              <h4 className="font-bold text-xs md:text-sm text-slate-900 dark:text-slate-100 font-display">
                                {gmailSelectedEmail.subject}
                              </h4>
                              <p className="text-[10px] font-semibold text-rose-700 dark:text-rose-405 mt-1">
                                From: <span className="font-mono">{gmailSelectedEmail.from}</span>
                              </p>
                            </div>
                            <span className="text-[10px] font-mono text-slate-405 text-right shrink-0">
                              {gmailSelectedEmail.date}
                            </span>
                          </div>

                          <div className="text-[11.5px] leading-relaxed whitespace-pre-wrap max-h-[300px] overflow-y-auto pr-1 bg-slate-50/20 dark:bg-slate-950/30 p-3 rounded-lg border border-slate-105 dark:border-slate-850">
                            {gmailSelectedEmail.body || gmailSelectedEmail.snippet || "(Empty Message)"}
                          </div>

                          <div className="flex items-center gap-2 pt-1.5 justify-end">
                            <button
                              onClick={() => {
                                handleTrashGmailMessage(gmailSelectedEmail.id);
                              }}
                              className="px-3.5 py-1.5 border border-rose-250 text-rose-700 dark:text-rose-300 hover:bg-rose-50 dark:hover:bg-rose-950 text-[11px] font-semibold rounded-lg cursor-pointer transition-all"
                            >
                              Move to Trash
                            </button>
                            <button
                              onClick={() => {
                                // Initialize compose as a reply
                                setGmailComposeTo(gmailSelectedEmail.fromEmail);
                                setGmailComposeSubject(`Re: ${gmailSelectedEmail.subject}`);
                                setGmailComposeBody(`\n\nOn ${gmailSelectedEmail.date}, ${gmailSelectedEmail.from} wrote:\n> ` + gmailSelectedEmail.snippet);
                                setShowGmailComposer(true);
                              }}
                              className="px-3.5 py-1.5 bg-rose-600 hover:bg-rose-700 text-white text-[11px] font-bold rounded-lg cursor-pointer transition-all"
                            >
                              Reply
                            </button>
                          </div>
                        ...
                        <div className={`p-6 border border-dashed rounded-xl text-center space-y-3 flex flex-col items-center justify-center min-h-[300px] ${themeClass("bg-slate-50/25 border-slate-200", "bg-slate-950/10 border-white/10", "bg-[#ebdcb9]/10 border-[#ebdcba]/40")}`}>
                          ...
                          <div className="space-y-1">
                            ...
                            <p className="text-[10.5px] text-slate-500 max-w-xs leading-relaxed">
                              Select a message from the list panel to view its full details, or click <strong>Compose Email</strong> to generate custom crisis reminders.
                            </p>
                          </div>
                        </div>
                      )}
              ...
              <div className={`border p-5 rounded-2xl relative overflow-hidden flex flex-col gap-3 transition-all duration-300 ${themeClass("bg-[#f0f4ff] border-[#dbe4ff]", "bg-white/[0.02]/20 border-white/10/20", "bg-[#ebdcb9]/40 border-[#d3c299]")}`}>
                ...
                <div className="flex flex-col gap-1.5 z-10 text-left">
                  ...
                  <h3 className={`text-base md:text-lg font-bold flex items-center gap-2 font-display ${themeClass("text-slate-900", "text-indigo-200", "text-[#3e2723]")}`}>
                    Self-Care Curation &amp; Insights
                  </h3>
                  <p className={`text-xs leading-relaxed max-w-2xl ${themeClass("text-slate-650", "text-slate-350", "text-[#5c4033]")}`}>
                    A bi-weekly clinical publishing column authored from the perspective of founder <strong>Manjishtha Pahilajani (Manji)</strong>. Integrating neuroscience, somatic anchoring pathways, and validated psychiatric citations to guide self-deescalation.
                  </p>
                </div>
              </div>

              {/* Founder Authorization / Upload Drawer */}
              <div className={`border rounded-2xl p-4.5 transition-all duration-300 text-left ${themeClass("bg-slate-50 border-slate-200", "bg-slate-950/40 border-slate-850", "bg-[#fcf8f0] border-[#ebdcb9]")}`}>
                <div className="flex items-center justify-between gap-3 flex-wrap">
                  <div className="flex items-center gap-2">
                    <span className="text-sm">🔑</span>
                    <h4 className="text-xs font-bold uppercase tracking-wider text-slate-800 dark:text-slate-200">
                      {isFounderAuthorized ? "Author Mode: Manjishtha Pahilajani (Manji)" : "Founder's Authoring Hub"}
                    </h4>
                  </div>
                  {!isFounderAuthorized ? (
                    <button
                    ...
                    <button
                      onClick={() => setIsFounderAuthorized(false)}
                      className="px-3 py-1 bg-rose-50 hover:bg-rose-100 text-rose-700 dark:text-rose-300 text-[10px] font-bold border border-rose-200 dark:border-rose-800 rounded-lg transition-colors cursor-pointer"
                    >
                      Sign Out of Author Suite
                    </button>
                  )}
                </div>
                ...
                {isFounderPanelOpen && !isFounderAuthorized && (
                  <div className="mt-4 p-3 bg-white dark:bg-black rounded-xl border border-dashed border-slate-300 dark:border-white/10 space-y-3">
                    ...
                    <div className="flex items-center gap-2">
                      <input
                        type="password"
                        placeholder="Enter Founder PIN (e.g., 1234)"
                        value={founderPIN}
                        onChange={(e) => setFounderPIN(e.target.value)}
                        className="flex-1 px-3 py-1.5 text-xs border rounded-lg bg-slate-55 dark:bg-slate-950 focus:outline-none focus:ring-1 focus:ring-indigo-400"
                        onKeyDown={(e) => {
                          if (e.key === 'Enter') {
                            if (founderPIN === "1234" || loginAlias?.toLowerCase() === "manji" || loginAlias?.toLowerCase() === "manjishtha") {
                              setIsFounderAuthorized(true);
                              setIsFounderPanelOpen(false);
                            } else {
                              alert("Incorrect Founder PIN. Please try entering '1234'.");
                            }
                          }
                        }}
                      />
                      <button
                        ...
                        className="px-3 py-1.5 bg-[#4359e6] hover:bg-[#3245c4] text-white font-bold text-xs rounded-lg cursor-pointer"
                      >
                        Authorize
                      </button>
                    </div>
                  ...
                  <div className="mt-4 space-y-4 pt-3 border-t border-dashed border-slate-300 dark:border-white/10 text-left">
                    ...
                    <div className="p-3.5 bg-indigo-505/10 dark:bg-white/[0.02]/10 border border-indigo-400/20 dark:border-indigo-805/30 rounded-xl space-y-2">
                      ...
                      <p className="text-[10.5px] text-slate-500 leading-normal">
                        Select which professional guide, character, or foundational standpoint you wish to write or auto-generate from:
                      </p>
                      <div className="relative">
                        <select
                    ...
                    {/* Option A: Generate automatically via AI */}
                    <div className="p-3.5 bg-emerald-500/5 border border-emerald-500/20 rounded-xl space-y-3">
                      <div className="flex items-center justify-between gap-2">
                        <h5 className="text-xs font-bold text-emerald-800 dark:text-emerald-305 flex items-center gap-1.5">
                          <span>✨ Auto-Generate Wisdom Post</span>
                          <span className="text-[9px] font-normal bg-emerald-500/10 text-emerald-705 px-1.5 py-0.2 rounded">Two posts a week program</span>
                        </h5>
                      </div>
                      <p className="text-[11px] text-slate-500 leading-relaxed">
                        Trigger the Antigravity synthesis engine to generate a fully cited, high-quality, clinical grounding blog post on a mental welfare theme written from your chosen perspective.
                      </p>
                      
                      <div className="space-y-2 text-left">
                        <label className="block text-[10px] font-bold text-slate-700 dark:text-slate-300 uppercase tracking-wider">Select or Write Topic:</label>
                        <div className="flex flex-wrap gap-1.5 mb-2">
                          {["Alleviating Parental Stress", "Somatic Grounding in ADHD", "Navigating Loneliness & Stress", "Processing Relational Grief", "Restoring Autonomic Calm"].map((theme) => (
                            <button
                              type="button"
                              key={theme}
                        ...
                        <div className="flex gap-2">
                          <input
                            type="text"
                            placeholder="Write your custom topic here..."
                            value={blogTopicInput}
                            onChange={(e) => setBlogTopicInput(e.target.value)}
                            className="flex-1 px-3 py-2 text-xs border rounded-lg bg-white dark:bg-slate-950 focus:outline-none focus:ring-1 focus:ring-emerald-500 text-slate-850 dark:text-slate-105"
                          />
                          <button
                            onClick={async () => {
                              if (!blogTopicInput.trim()) {
                                alert("Please select or enter an active topic first.");
                                return;
                              }
...
Formatting: Use clean, structured Markdown. Make it professional, authoritative, but beautifully comforting and accessible. Maintain this professional standard completely.`;
                            ...
                            className={`px-3 py-2 bg-emerald-600 hover:bg-[#047857] text-white font-bold text-xs rounded-lg transition-all flex items-center gap-1.5 cursor-pointer ${isGeneratingBlog ? "opacity-50 cursor-not-allowed" : ""}`}
                          >
                            {isGeneratingBlog ? "Writing..." : "✨ Auto-Cure Insights"}
                          </button>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="p-3.5 bg-indigo-500/5 border border-indigo-500/20 rounded-xl space-y-3 text-left">
                      <h5 className="text-xs font-bold text-indigo-800 dark:text-indigo-305">📤 Upload and Publish Custom Manual Column</h5>
                      <div className="space-y-2">
                        <input
                          type="text"
                          ...
                          className="w-full px-3 py-1.5 text-xs border rounded-lg bg-white dark:bg-slate-950 text-slate-850 dark:text-slate-105 focus:outline-none focus:ring-1 focus:ring-indigo-500"
                        />
                        <textarea
                          placeholder={`Write your article in Markdown / Text format from ${selectedAuthorPerspective}'s POV...`}
                          rows={4}
                          value={customBlogContent}
                          onChange={(e) => setCustomBlogContent(e.target.value)}
                          className="w-full px-3 py-1.5 text-xs border rounded-lg bg-white dark:bg-slate-950 text-slate-850 dark:text-slate-105 focus:outline-none focus:ring-1 focus:ring-indigo-500 font-mono text-[11px]"
                        />
                        <div className="flex items-center gap-2">
                          <input
                            type="text"
                            placeholder="Tags (comma-separated, e.g. Anxiety, CBT)"
                            value={customBlogTags}
                            onChange={(e) => setCustomBlogTags(e.target.value)}
                            className="flex-1 px-3 py-1.5 text-xs border rounded-lg bg-white dark:bg-slate-950 text-slate-850 dark:text-slate-105 focus:outline-none"
                          />
                          <button
                            onClick={() => {
                              if (!customBlogTitle.trim() || !customBlogContent.trim()) {
                                alert("Please fill in both the title and body content.");
                                return;
                              }
                              const parsedTags = customBlogTags.split(",").map(t => t.trim()).filter(Boolean);
                              const newPost = {
                                id: `custom-blog-${Date.now()}`,
                                title: customBlogTitle,
                                author: selectedAuthorPerspective,
                                date: new Date().toLocaleDateString("en-US", { year: 'numeric', month: 'long', day: 'numeric' }),
                                tags: parsedTags.length ? parsedTags : ["Founder"],
                                content: `# ${customBlogTitle}\n*By ${selectedAuthorPerspective}*\n*Published Column — ${new Date().toLocaleDateString("en-US", { year: 'numeric', month: 'long', day: 'numeric' })}*\n\n${customBlogContent}`
                              };
                              setBlogsList([newPost, ...blogsList]);
                              setCustomBlogTitle("");
                              setCustomBlogContent("");
                              setBlogStatusMessage({ type: 'success', text: `📤 Successfully published "${customBlogTitle}" to the Self-Care feed!` });
                            }}
                            className="px-4 py-1.5 bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-bold rounded-lg cursor-pointer transition-colors"
                          >
                            Publish
                          </button>
                        </div>
                ...
                {blogsList.map((blog) => (
                  ...
                  >
                    ...
                    <div className="flex flex-wrap items-center justify-between gap-2 text-[10.5px] font-sans text-slate-440 mb-2 text-left">
                      <span className="font-medium text-emerald-650 flex items-center gap-1">
                        <span className="w-1.5 h-1.5 rounded-full bg-emerald-500"></span>
                        {blog.author}
                      </span>
                      <div className="flex items-center gap-3">
                    ...
                    <h5 className={`text-base font-bold font-display underline decoration-emerald-500/20 underline-offset-4 tracking-tight leading-snug text-left ${themeClass("text-slate-900", "text-white", "text-[#3e2723]")}`}>
                      {blog.title}
                    </h5>
                    ...
                    {/* Content Renderer */}
                    <div className="prose prose-sm prose-indigo max-w-none text-left">
...
I am speaking to you now as **${CHARACTERS.find(c => c.id === pendingCharId)?.name}**.`,
                        timestamp: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })
                      }
                    ]);
                    setPendingCharId(null);
